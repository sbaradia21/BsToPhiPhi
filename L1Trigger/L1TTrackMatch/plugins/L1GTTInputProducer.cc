...
private:
  ...
  enum ConversionBitWidths {
    ...
    kPTMagSize = 9, //7 //9 //7   // magnitude output size; MAG + FRAC should be <= kPTInputSize
    kPTFracSize = 5, //3 //5 //7  // fraction output size; MAG + FRAC should be <= kPTInputSize
  ...
  TH1D *trackPosPtGTTH_, *trackPosPtEmuGTTH_, *trackPosPtDiffGTTH_, *trackPosPtDiffGTT1H_;
  TH1D *trackNegPtGTTH_, *trackNegPtEmuGTTH_, *trackNegPtDiffGTTH_, *trackNegPtDiffGTT1H_;
...
// member functions
//
void L1GTTInputProducer::beginJob() {
  edm::Service<TFileService> fs;
  if (!fs.isAvailable()) return;
  trackPosPtGTTH_ = fs->make<TH1D>("trackPosPtGTT", "pt",  320, 0, 10);
  trackPosPtEmuGTTH_ = fs->make<TH1D>("trackPosPtEmuGTT", "pt emu",  320, 0, 10);
  trackPosPtDiffGTTH_ = fs->make<TH1D>("trackPosPtDiffGTT", "Difference of pt abs",100, 0, 0.5);
  trackPosPtDiffGTT1H_ = fs->make<TH1D>("trackPosPtDiffGTT1", "Difference of pt",200, -0.5, 0.5);

  trackNegPtGTTH_ = fs->make<TH1D>("trackNegPtGTT", "abs pt",  320, 0, 10);
  trackNegPtEmuGTTH_ = fs->make<TH1D>("trackNegPtEmuGTT", "pt emu",  320, 0, 10);
  trackNegPtDiffGTTH_ = fs->make<TH1D>("trackNegPtDiffGTT", "Difference of pt abs",100, 0, 0.5);
  trackNegPtDiffGTT1H_ = fs->make<TH1D>("trackNegPtDiffGTT1", "Difference of pt",200, -0.5, 0.5);
}
/**
...
void L1GTTInputProducer::generate_pt_lut() {
  ...
  std::cout << "generate_pt_lut::The pt_lut_[" << kPtLutSize << "] values are "<<std::endl;
  // iterate through all values in the lut
...
void L1GTTInputProducer::produce(edm::StreamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  for (const auto& track : *l1TracksHandle) {
    ...
    double charge = 1. - (2 * chargeBit.to_uint());
    double diff = expectedPt - (charge * ptBits.to_double()); //Sim - Emu
    //expectedPt has the charge in it
    //ptBits only magnitude ...thus multiplying with charge when calculating diff
    if(charge > 0) {
      trackPosPtGTTH_->Fill(expectedPt);
      trackPosPtEmuGTTH_->Fill(ptBits.to_double());
      trackPosPtDiffGTTH_->Fill(std::abs(diff));
      trackPosPtDiffGTT1H_->Fill(diff);
    }
    else
      {
	trackNegPtGTTH_->Fill(std::abs(expectedPt));
	trackNegPtEmuGTTH_->Fill(ptBits.to_double());
	trackNegPtDiffGTTH_->Fill(std::abs(diff));
	trackNegPtDiffGTT1H_->Fill(diff);
      }
  }
